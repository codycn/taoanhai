                ...
                >
                 ...
                 {onShare && image.is_public && (
                    <div className="text-center text-xs font-semibold text-green-400 bg-green-500/10 py-2 rounded-lg">
                        {t('modaimport React, { useState } from 'react';
import { GalleryImage } from '../../types';
import { useAuth } from '../../contexts/AuthContext';
import { getRankForLevel } from '../../utils/rankUtils';
import { useTranslation } from '../../hooks/useTranslation';
...
const ImageModal: React.FC<ImageModalProps> = ({ isOpen, onClose, image, showInfoPanel = true, onShare }) => {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    // This single function will handle both direct and proxy download logic.
    const attemptDownload = async () => {
        try {
            // First attempt: Direct fetch (works if CORS is configured on R2)
            const response = await fetch(image.image_url);
            if (!response.ok) {
                // If it fails for reasons other than CORS (e.g., 404), it will be caught below.
                // A CORS failure will also throw and be caught.
                throw new Error('Direct fetch failed.');
            }
            return response.blob();
        } catch (directError) {
            console.warn('Direct download failed, trying proxy...', directError);
            // Second attempt: Fetch via our Netlify function proxy
